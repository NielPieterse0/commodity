...
- Frozen WORK-359 handoff still reports `change_id=null` although Work Management correctly reports `change_id=359-stacking-policy`. This run re-read the immutable handoff and reproduced `bind_task_handoff_change(work_id=WORK-359, source_path=<governed worktree>) -> CANDIDATE_SOURCE_INVALID: source is not a KIS checkout`; binding the inner change-record path returns `CANDIDATE_SOURCE_NOT_GOVERNED`, while `validate_change_claims` simultaneously reports two active governed changes and zero orphaned change worktrees. Attempting to rematerialize the immutable handoff with `change_id=359-stacking-policy` correctly fails `HANDOFF_CONTRACT_IMMUTABLE`. This isolates the later promotion blocker to KIS handoff source recognition rather than Commodity change governance. It does not block current implementation/scoring but must be retried or repaired before PromotionReady if that path requires the frozen handoff binding.
