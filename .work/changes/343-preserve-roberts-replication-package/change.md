...
- The same local verification run completed `437 passed` tests and `6 failed`; all six failures are the Windows Application Control block on the worktree-local `pyarrow._parquet` DLL in Databento-provider tests. No changed path is involved. Exact-head CI remains required for a clean cross-environment pass. These operational claims are bound in `verification-evidence.json` to commit `976ca1ba38df9e7ab4f0d7518b01e27764b1a0bf`.
- Initial specialist review raised one procedural finding because `acquisition-evidence.json` was untracked at review time. Commit `976ca1ba38df9e7ab4f0d7518b01e27764b1a0bf` resolved it, and the exact-commit code-quality review returned no findings.
