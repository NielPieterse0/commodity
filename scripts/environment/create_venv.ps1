...
$repoRoot = (Resolve-Path -LiteralPath (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))).Path.TrimEnd('\')
...
if (Test-Path -LiteralPath $candidateVenv) {
    ...
    if (-not (Test-Path -LiteralPath $existingConfig -PathType Leaf) -or -not (Test-Path -LiteralPath $venvPython -PathType Leaf)) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Existing repository virtual environment uses a different base interpreter: $candidateVenv"
    }
} else {
