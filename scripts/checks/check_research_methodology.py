...
def check_schema() -> None:
    ...
    validate_document(ROOT / "contracts/programme_evidence.schema.json", ROOT / "research/programme/programme_evidence_map.json")
    validate_document(ROOT / "contracts/programme_inference.schema.json", ROOT / "research/programme/programme_inference_ledger.json")
    validate_document(ROOT / "contracts/sealed_windows.schema.json", ROOT / "research/programme/sealed_windows.json")
    ...
    validate_document(ROOT / "contracts/revisit_triggers.schema.json", ROOT / "research/programme/research_revisit_triggers.json")
    assert_revisit_preflight_current(load_json(ROOT / "research/programme/research_revisit_triggers.json"))
...
def check_programme_inference() -> None:
    ledger = load_json(ROOT / "research/programme/programme_inference_ledger.json")
    validate_inference_ledger(ledger)
    sealed = load_json(ROOT / "research/programme/sealed_windows.json")
    ids: set[str] = set()
