...
For each governed change, keep its scope, plan, tasks, hypotheses where applicable, decisions, implementation notes, review findings, verification evidence, interpretation, and closeout in one change-local record under `.work/changes/<issue>-<slug>/`. The authoritative schema-v4 change scope classifies complexity, risk triggers, and `work_management.documentation_impact`; that classification determines whether separate documentation work is required. For governed research implementation, the approved L3 experiment/preregistration is the scientific and implementation specification: do not generate a second implementation spec in `.work`. The `.work` plan/task record only translates that experiment contract into repository changes, TDD steps, review, verification, and evidence, and MUST reference rather than duplicate or redefine the scientific design. The linked GitHub Project issue is the human-facing research-slice document. New slice-specific repository artifacts SHOULD be machine-readable JSON, schemas, contracts, or generated evidence; do not create per-slice Markdown merely to narrate the slice. Repository-level generated Markdown remains appropriate when a canonical owner requires a human projection. Documentation is added or reconciled only when the classified documentation impact or changed canonical owner requires it; documentation classification never creates a second scientific/implementation specification. Active, closed, held, and deferred are lifecycle states, not directory classes: closing a governed change does not relocate its record. When an associated implementation worktree is retained, it belongs under `.work/worktrees/<issue>-<slug>/`; safe worktree cleanup may remove the physical checkout later without removing the change record.
