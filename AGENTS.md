...
| 10 | Research hierarchy and experiment contracts | `contracts/programme.schema.json`, `contracts/research_line.schema.json`, `contracts/legacy_experiment_record.schema.json`, `contracts/prereg.schema.json`, `contracts/results.schema.json`, `contracts/interpretation_metadata.schema.json`, `contracts/experiment_record.schema.json` |
| 11 | L1 programme identity and child-line ownership | `research/programmes/<programme-id>/programme.json`; programme directories use stable numbered identities and own their L2 line references |
| 12 | L2 research-line identity and child-experiment ownership | `research/programmes/<programme-id>/lines/<research-line-id>/line.json`; each line declares its programme parent and experiment children |
| 13 | Experiment evidence and durable knowledge | `research/programmes/<programme-id>/lines/<research-line-id>/experiments/<experiment-id>/`; current 15-step experiments use prereg/results/record artifacts, while recovered pre-15-step work uses `legacy-record.json` plus hash-bound preserved legacy sources without retrospective preregistration; the linked GitHub Project issue is the human-facing slice record |
...
| 15 | Programme evidence and feasibility state | `research/programmes/<programme-id>/evidence-map.json` |
| 16 | Programme inference ledger | `research/programmes/<programme-id>/inference-ledger.json` |
| 17 | Programme decisions, backlog, sealed windows, and revisit state | `research/programmes/<programme-id>/decisions.json`, `backlog.json`, `sealed-windows.json`, `revisit-triggers.json` |
| 18 | Exploratory/diagnostic run contract | `contracts/exploratory_run.schema.json`; programme-specific research evidence belongs beneath its owning programme/line, while change-only operational records belong to the governed change record |
| 19 | Literature snapshot contract | `contracts/literature_snapshot.schema.json`; active programme/line/experiment-specific literature belongs beneath the corresponding `research/programmes/<programme-id>/` lineage; retired methodology literature belongs in historical evidence |
| 20 | HOLD/DEFER scientific revisit triggers and evaluation history | `contracts/revisit_triggers.schema.json`, `research/programmes/<programme-id>/revisit-triggers.json` |
...
| 33 | Human-readable research methodology projection | `config/research_methodology.json`, `config/research_dataset.json`, and applicable `contracts/*.schema.json`; projected as generated `docs/research-methodology.md` |
| 34 | L0 repository mandate human projection | `AGENTS.md` owns the mandate; projected deterministically as generated `docs/big-picture.md` |
| 35 | Retired raw legacy definitions and non-authoritative source material | `.work/historical/config/`, `.work/historical/contracts/`; curated scientific legacy experiment lineage belongs under the owning programme/line experiment directory via `legacy-record.json` |
...
For each governed change, keep its scope, plan, tasks, hypotheses where applicable, decisions, implementation notes, review findings, verification evidence, interpretation, and closeout in one change-local record under `.work/changes/<issue>-<slug>/`. The authoritative schema-v4 change scope classifies complexity, risk triggers, and `work_management.documentation_impact`; that classification determines which KIS change artifacts and reviews are required. For ordinary engineering/documentation/configuration changes, follow the live KIS classification and artifact requirements directly.

Research uses a nested workflow. Commodity owns iterative scientific progression before and after bounded repository implementation: L0-L3 literature, mechanism, hypotheses, feasibility, source/PIT semantics, power/MEPI, GO/HOLD/REDESIGN, post-result interpretation, and programme consolidation may loop as evidence requires. Crossing the explicit `implementation_ready` boundary starts or resumes a bounded KIS change. The approved L3 research artifact/preregistration remains the scientific requirements authority. When KIS requires `spec.md` for a medium/large research-originated change, that spec is only a thin science-to-repository implementation mapping: it MUST reference the exact research authority and MUST NOT restate, duplicate, reinterpret, or silently extend the scientific design. Small KIS changes use the compact artifact set selected by live KIS.

Engineering iteration that preserves the scientific contract stays inside the same KIS change. If implementation exposes a material scientific ambiguity, estimand/data-semantics change, feasibility/power failure, or other research-design change, leave the implementation loop and return to the owning L3 research state. Preserve all KIS evidence whose declared validity inputs still hold; invalidate/rerun only stale evidence. Resume implementation only from the updated/new scientific identity required by the methodology. After KIS PromotionReady/merge/cleanup, the research lineage consumes the exact implementation/landing identities and continues L4/L5 execution, interpretation, and L5→L1/L2 consolidation without recreating KIS closeout evidence.

The linked GitHub Project issue is the human-facing research-slice document. New slice-specific repository artifacts SHOULD be machine-readable JSON, schemas, contracts, or generated evidence; do not create per-slice Markdown merely to narrate the slice. Repository-level generated Markdown remains appropriate when a canonical owner requires a human projection. Documentation is added or reconciled only when the classified documentation impact or changed canonical owner requires it. Active, closed, held, and deferred are lifecycle states, not directory classes: closing a governed change does not relocate its record. When an associated implementation worktree is retained, it belongs under `.work/worktrees/<issue>-<slug>/`; safe worktree cleanup may remove the physical checkout later without removing the change record.

`.work/changes/` is versioned governed development history for bounded change plans, findings, review records, and closeout evidence. It is operational history, not product or scientific authority. Runtime behavior, configuration, tests, governed research records, artifacts, and maintained product documentation MUST NOT depend on it.

`.work/worktrees/` and other explicitly ignored `.work` runtime, cache, scratch, quarantine, download, checkpoint, source-clone, and temporary areas are local disposable execution state. Scientific evidence and machine authority MUST NOT depend on ignored `.work` state. Raw or ignored snapshots may serve as evidence inputs, but only safe provenance summaries required by authoritative records should be committed.
...
- Code review for a governed change MUST run from that change's isolated governed worktree, never from repository `main` or another checkout. Use the live KIS `review_change_with_agent` review operation directly against the exact review source; do not start PR preparation merely to obtain implementation review evidence.
- Prefer an exact staged or immutable base-to-head range review that contains only the intended review surface. For large changes, stage and review independently reviewable source and test surfaces separately when that is the established KIS pattern; restore/stage the full governed change only after those reviews are complete.
- If KIS reports `manual_fallback` with `mode=exact-diff` because bounded review evidence cannot include a large source file, follow that declared fallback: review the exact source-bound diff, retain the durable review evidence/closure in the governed change record, and continue through the registered KIS PR path. Do not repeatedly retry an impossible whole-change projector, fabricate review closure, weaken the gate, or treat projector-size failure as an implementation defect.
- Before PR creation, confirm the review evidence is bound to the same isolated worktree/change identity and exact staged/range/commit source that will be committed or reconciled. After implementation review closure, use KIS registered reconciliation/PR, exact-head CI, merge, refresh, and cleanup operations; repository-required GitHub review is distinct from this implementation review.
