...
def test_authoritative_config_owns_availability_rules_without_unlocking_massive() -> None:
    ...
    massive = cfg["sources"]["massive_henry_hub_evaluation"]
    canonical = cfg["sources"][cfg["canonical_market_source_id"]]
    assert massive["non_display_backtesting_rights_verified"] is False
    assert massive["backtest_evidence_allowed"] is False
    assert canonical["provider"] == "databento_futures"
    assert canonical["non_display_backtesting_rights_verified"] is True
    assert canonical["backtest_evidence_allowed"] is True
