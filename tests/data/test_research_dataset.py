...
DATABENTO_SOURCE = "databento_henry_hub"
MASSIVE_EVALUATION_SOURCE = "massive_henry_hub_evaluation"
...
def test_canonical_dataset_consumes_provider_neutral_contract_rows(monkeypatch) -> None:
    ...
    dataset, manifest = build_pit_dataset(
        ...
        market_source_id=DATABENTO_SOURCE,
    )
    assert not dataset.empty
...
def test_canonical_target_skips_cross_contract_roll_return(monkeypatch) -> None:
    ...
    dataset, _ = research_dataset.build_pit_dataset(
        None, evidence_mode="canonical", canonical_contracts=contracts, market_source_id=DATABENTO_SOURCE
    )
...
def test_canonical_dataset_fails_closed_without_required_curve_ranks(monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="M1-M4 market structure"):
        research_dataset.build_pit_dataset(
            None, evidence_mode="canonical", canonical_contracts=_canonical_contracts(), market_source_id=DATABENTO_SOURCE
        )
...
def test_contract_market_dataset_requires_explicit_source_identity() -> None:
    from commodity.research_dataset import build_pit_dataset

    with pytest.raises(ValueError, match="explicit market_source_id"):
        build_pit_dataset(
            None,
...
def test_evaluation_dataset_uses_contract_market_structure_without_promotion_rights() -> None:
    ...
    dataset, manifest = build_pit_dataset(
        ...
        evidence_mode="evaluation_pit",
        canonical_contracts=_canonical_contracts_four(),
        market_source_id=MASSIVE_EVALUATION_SOURCE,
    )
    assert not dataset.empty
    assert manifest["market_source_id"] == MASSIVE_EVALUATION_SOURCE
    assert manifest["market_provider"] == "massive_futures"
...
def test_canonical_roll_safe_features_fail_when_new_contract_has_no_prior_history(monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="selected-contract prior-session history"):
        research_dataset.build_pit_dataset(
            None, evidence_mode="canonical", canonical_contracts=deprived, market_source_id=DATABENTO_SOURCE
        )
