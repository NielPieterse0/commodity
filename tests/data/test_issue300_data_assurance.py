...
from commodity.providers.massive_futures import normalize_massive_contract_history
...
def test_research_pit_derives_literal_conservative_market_availability() -> None:
    dataset, manifest = build_pit_dataset(
        _market(), required_families=("market", "calendar_seasonality")
    )
    assert dataset.index[0].hour == 23 and dataset.index[0].minute == 59
    assert manifest["prediction_timestamp_semantics"] == (
        "explicit_or_conservatively_derived_market_available_at_cutoff"
    )
    assert manifest["data_assurance"]["reconstruction_status"] == (
        "pending_reconstruction_verification"
    )
    with pytest.raises(DataAssuranceError, match="not verified"):
        assert_research_ready(manifest["data_assurance"])


def test_exact_second_build_verifies_reconstruction_but_not_semantics() -> None:
    first, first_manifest = build_pit_dataset(
        _market(), required_families=("market", "calendar_seasonality")
    )
    second, second_manifest = build_pit_dataset(
        _market().copy(), required_families=("market", "calendar_seasonality")
    )
    assurance = verify_reconstruction_pair(
        first,
        first_manifest["data_assurance"],
        second,
        second_manifest["data_assurance"],
    )
    assert assurance["reconstruction_status"] == "verified"
    assert assurance["reconstruction_independence"] == "same_implementation_repeatability_only"
    assert assurance["semantic_status"] == "pending_semantic_verification"
    with pytest.raises(DataAssuranceError, match="semantic correctness"):
        assert_research_ready(assurance)


def test_explicit_semantic_verification_upgrades_reconstruction_to_research_ready() -> None:
    first, first_manifest = build_pit_dataset(
        _market(), required_families=("market", "calendar_seasonality")
    )
    second, second_manifest = build_pit_dataset(
...
def test_semantic_verification_rejects_unbound_self_certified_booleans() -> None:
    first, first_manifest = build_pit_dataset(
        _market(), required_families=("market", "calendar_seasonality")
    )
    second, second_manifest = build_pit_dataset(
    ...
    with pytest.raises(DataAssuranceError, match="bound verifier reference"):
        verify_semantic_assurance(
            reconstruction,
            semantic_evidence={
                "method": "explicit_dataset_semantics_v1",
                "checks": {"self_certified": True},
            },
        )


def test_semantic_verification_fails_closed_on_any_failed_check() -> None:
    first, first_manifest = build_pit_dataset(
        _market(), required_families=("market", "calendar_seasonality")
    )
    second, second_manifest = build_pit_dataset(
    ...
    with pytest.raises(DataAssuranceError, match="explicit passing checks"):
        verify_semantic_assurance(
            reconstruction,
            semantic_evidence={
                "method": "explicit_dataset_semantics_v1",
                "checks": {"market_semantics_reviewed": True, "pit_semantics_reviewed": False},
            },
        )
...
def test_data_assurance_is_tamper_evident() -> None:
    ...
    broken = json.loads(json.dumps(assurance))
...
def test_phase_d_v1_is_preserved_as_known_defective_evidence() -> None:
    methodology = json.loads(
        (Path(__file__).resolve().parents[2] / "config" / "research_methodology.json").read_text(
            encoding="utf-8"
        )
    )
