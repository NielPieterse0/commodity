...
def _manifest(frame: pd.DataFrame) -> dict:
    ...
    construction = {
        ...
        "layers": [
            {"name": "retained_source_evidence", "status": "constructed", "sha256": "1" * 64},
            {"name": "canonical_normalization", "status": "constructed", "sha256": "2" * 64},
            {"name": "pit_availability", "status": "constructed", "sha256": "3" * 64},
...
def test_freeze_is_content_addressed_and_repeatable(tmp_path: Path) -> None:
    ...
    assert frozen["configuration_sha256"]["research_dataset"]
...
def test_freeze_round_trip_preserves_index_name(tmp_path: Path) -> None:
    from commodity.dataset_freeze import freeze_full_v1_dataset, load_frozen_dataset
    ...
    restored, manifest = load_frozen_dataset(frozen)

    pd.testing.assert_frame_equal(restored, frame, check_freq=False)
    assert manifest["index_name"] == "available_at"


def test_freeze_round_trip_preserves_numeric_dtypes(tmp_path: Path) -> None:
    from commodity.dataset_freeze import freeze_full_v1_dataset, load_frozen_dataset

    frame = _dataset()
    frame["power_signal"] = np.arange(len(frame), dtype=float)
    frozen = freeze_full_v1_dataset(frame, _manifest(frame), tmp_path)

    restored, manifest = load_frozen_dataset(frozen)

    pd.testing.assert_frame_equal(restored, frame, check_freq=False)
    assert manifest["column_dtypes"]["power_signal"] == "float64"
...
def test_freeze_hashes_the_config_files_resolved_at_runtime(monkeypatch, tmp_path: Path) -> None:
    ...
    override.mkdir()
    for name in (
        "research_dataset.json", "data_sources.json", "assumptions.json", "research_methodology.json"
    ):
...
def test_freeze_binds_all_material_transform_modules(tmp_path: Path) -> None:
    ...
    expected = {
        ...
        "dataset_freeze", "data_assurance", "full_v1",
    }
