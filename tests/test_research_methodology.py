...
def sample_prereg() -> dict:
    return {
        ...
        "slice_id": "slice-a",
        "evidence_scan_ref": "programme-evidence-map:2026-08-29",
        "literature_snapshot_ref": "literature:storage-v1",
        ...
        "datasets": [{"id": "d1", "vintage": "v1", "role": "research_oos"}],
        ...
        "coherence_triggers": [
            {"id": "implausibly_large_benchmark_improvement", "direction": "unexpectedly_good"},
            {"id": "mechanism_sign_conflict", "direction": "unexpectedly_bad"},
            {"id": "few_observation_concentration", "direction": "both"},
        ],
...
def test_authority_map_names_new_future_experiment_owners() -> None:
    ...
    assert "config/programme_inference_ledger.json" in agents
    assert "config/sealed_windows.json" in agents
    assert "Completed V1/V2 experiment records remain legacy evidence" in agents
...
def test_remote_prereg_binding_requires_clean_repository(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(methodology, "_git_text", lambda _root, *args: " M config/programme_inference_ledger.json" if args[:2] == ("status", "--porcelain") else "")
