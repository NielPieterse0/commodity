...
@pytest.mark.parametrize("adapter_type", [kronos.KronosMiniAdapter, kronos.KronosCheckpointAdapter])
def test_forecast_rejects_overlapping_or_backward_horizon(adapter_type) -> None:
    adapter = adapter_type.__new__(adapter_type)
    adapter.predictor = object()
    adapter.inference = {"T": 1.0, "top_p": 0.9, "sample_count": 1, "verbose": False}
    history_index = pd.date_range("2026-01-01", periods=2, tz="UTC")
    history = pd.DataFrame(
        {"open": 1.0, "high": 1.0, "low": 1.0, "close": 1.0, "volume": 1.0},
        index=history_index,
    )
    future = pd.DatetimeIndex([history_index[-1]])
    with pytest.raises(kronos.KronosArtifactError, match="strictly after history"):
        adapter.forecast(history, future)


def test_forecast_rejects_non_finite_predictor_output() -> None:
    class Predictor:
        def predict(self, **_kwargs):
            return pd.DataFrame({"close": [float("nan")]})

    adapter = kronos.KronosMiniAdapter.__new__(kronos.KronosMiniAdapter)
    ...
    future = pd.date_range("2026-01-02", periods=1, tz="UTC")
    with pytest.raises(kronos.KronosArtifactError, match="non-finite"):
        adapter.forecast(history, future)
