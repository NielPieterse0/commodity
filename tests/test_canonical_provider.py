...
from commodity.canonical_provider import ProviderLoadError, load_canonical_provider
...
def test_provider_loader_resolves_adapter_by_convention(monkeypatch) -> None:
    ...
    monkeypatch.setattr("commodity.canonical_provider.importlib.import_module", fake_import)
    assert load_canonical_provider("example_futures") is sentinel
    assert imported == ["commodity.example_futures_provider"]
...
def test_provider_loader_fails_closed_when_adapter_has_no_factory(monkeypatch) -> None:
    monkeypatch.setattr(
        "commodity.canonical_provider.importlib.import_module", lambda _: object()
    )
