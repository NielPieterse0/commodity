...
def test_programme_ledger_registration_is_append_only_by_identity() -> None:
    ...
    ledger = {"schema_version": 1, "programme_id": "001-commodity-natural-gas", "entries": [], "family_inference": []}
...
def test_interpretation_binds_both_scientific_artifacts() -> None:
    ...
    metadata = {
        ...
        "post_result_literature_snapshot_ref": {
            "path": "tests/fixtures/literature-snapshot-post.json",
            "sha256": hashlib.sha256((ROOT / "tests/fixtures/literature-snapshot-post.json").read_bytes()).hexdigest(),
        },
        "external_triangulation": "Independent literature does not override the governed evidence threshold.",
        "literature_expectation_assessment": "The observed direction was compatible with the literature-derived mechanism, but the magnitude did not clear the preregistered practical threshold.",
        "coherence_interpretation": "No predeclared coherence trigger changed the interpretation of the preserved result.",
        "hierarchy_retrace": {
            "L4": "The run produced weak governed evidence under the frozen implementation.",
            "L3": "The exact hypothesis did not clear its preregistered practical threshold.",
            "L2": "This weakens this storage slice without rejecting the broader storage line.",
            "L1": "Programme belief changes only narrowly after accounting for prior attempts and reused history.",
            "L0": "The result informs how L1 serves the fixed Commodity repository mandate; it does not redefine L0.",
        },
    }
...
def test_confirmatory_execution_requires_exact_frozen_preregistration() -> None:
    ...
    ledger = register_inference_entry({"schema_version": 1, "programme_id": "001-commodity-natural-gas", "entries": [], "family_inference": []}, prereg)
    freeze = {
        "schema_version": 1,
        "experiment_id": prereg["experiment_id"],
        "frozen": True,
...
def test_build_results_cli_requires_exact_execution_gate_inputs() -> None:
    ...
    assert args.programme_evidence is None
    assert args.sealed_registry is None
...
def test_build_results_rejects_invalid_schema_before_writing_immutable_artifact(tmp_path) -> None:
    ...
    ledger = register_inference_entry(
        {"schema_version": 1, "programme_id": "001-commodity-natural-gas", "entries": [], "family_inference": []},
        prereg,
    )
    freeze = {
        "schema_version": 1,
