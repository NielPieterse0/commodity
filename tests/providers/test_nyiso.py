...
def test_normalize_p7_archive_preserves_vintage_and_conservative_availability() -> None:
    ...
    assert frame["source_id"].eq("nyiso_p7_iso_load_forecast").all()
...
def test_nyiso_month_starts_accept_timezone_aware_bounds_without_warning() -> None:
    import warnings

    from commodity.nyiso import _month_starts

    with warnings.catch_warnings():
        warnings.simplefilter("error")
        months = _month_starts(
            pd.Timestamp("2024-08-13T23:59:00Z"),
            pd.Timestamp("2024-10-01T00:00:00Z"),
        )
    assert list(months) == list(pd.date_range("2024-08-01", "2024-10-01", freq="MS"))


def test_nyiso_v1_window_capture_is_resumable_and_loader_requires_daily_coverage(
    tmp_path: Path,
) -> None:
    class Client:
        def __init__(self) -> None:
    ...
    manifests_again = capture_nyiso_v1_window(
        client,
        "2024-08-13",
        "2024-08-14",
        tmp_path,
    ...
    assert manifests_again == manifests
    assert client.calls == 1

    frame = load_nyiso_v1_window(tmp_path, "2024-08-13", "2024-08-14")
    assert len(frame) == 2
    assert list(frame["observed_for"].dt.tz_convert("America/New_York").dt.date.astype(str)) == [
        "2024-08-13",
        "2024-08-14",
    ]
    assert frame["source_id"].eq("nyiso_p7_iso_load_forecast").all()
