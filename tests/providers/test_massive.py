...
from commodity.providers.massive_futures import (
...
def test_massive_archive_reconstructs_canonical_exactly_from_retained_raw_artifacts(tmp_path) -> None:
    from commodity.providers import massive_futures
    ...
    assert payload["source_component_hash_algorithm"] == "canonical-records-column-order-invariant-v1"
    reconstructed = massive_futures.reconstruct_massive_archive(manifest, _schema())
    stored = pd.read_csv(manifest.parent / "canonical.csv")
    stored = __import__("commodity.market_data", fromlist=["validate_contract_history"]).validate_contract_history(stored, _schema())
    pd.testing.assert_frame_equal(reconstructed, stored, check_exact=True)


def test_massive_archive_reconstruction_accepts_undeclared_legacy_component_hash(tmp_path) -> None:
    import json
    ...
    class Client:
        ...
        def fetch_session_aggregates(self, ticker, start_trade_date, end_trade_date):
            return pd.DataFrame(
                [{"ticker": ticker, "session_end_date": "2025-01-02", "settlement_price": 3.08, "volume": 1234}]
            )

    manifest = capture_massive_archive(
        Client(), _schema(), "NG", "2025-01-01", "2025-01-31",
        "2026-08-13T08:00:00Z", tmp_path, "legacy-component-hash",
    )
    payload = json.loads(manifest.read_text(encoding="utf-8"))
    payload.pop("source_component_hash_algorithm", None)
    payload["source_contracts"][0]["source_sha256"] = "f" * 64
...
def test_massive_archive_reconstruction_rejects_self_consistent_canonical_drift(tmp_path) -> None:
    import hashlib
    import json
    ...
    manifest = capture_massive_archive(
        Client(), _schema(), "NG", "2025-01-01", "2025-01-31",
        "2026-08-13T08:00:00Z", tmp_path, "drifted",
    )
    canonical = pd.read_csv(manifest.parent / "canonical.csv")
    canonical.loc[0, "settle"] += 0.01
    content = canonical.to_csv(index=False).encode("utf-8")
    (manifest.parent / "canonical.csv").write_bytes(content)
    payload = json.loads(manifest.read_text(encoding="utf-8"))
    artifact = next(item for item in payload["artifacts"] if item["path"] == "canonical.csv")
    artifact["bytes"] = len(content)
    artifact["sha256"] = hashlib.sha256(content).hexdigest()
    manifest.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8", newline="")

    with pytest.raises(DataContractViolation, match="reconstruction differs"):
        massive_futures.reconstruct_massive_archive(manifest, _schema())


def test_massive_provider_factory_applies_configured_pacing() -> None:
    from commodity.providers.massive_futures import create_provider
