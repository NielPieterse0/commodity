...
def test_ci_runs_public_hygiene_on_full_pr_history() -> None:
    ...
    assert "python scripts/checks/check_public_hygiene.py" in ci
...
def test_public_hygiene_script_passes_current_tree() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/checks/check_public_hygiene.py"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,
...
def test_closing_keyword_guard_is_present() -> None:
    script = (ROOT / "scripts" / "checks" / "check_public_hygiene.py").read_text(encoding="utf-8")
...
def test_closing_keyword_guard_covers_github_reference_forms() -> None:
    closing = runpy.run_path(str(ROOT / "scripts" / "checks" / "check_public_hygiene.py"))["CLOSING"]
    for text in (
        "Fixes #123",
