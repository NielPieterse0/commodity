...
@pytest.mark.parametrize("wrapper", ["Write-Output", "echo"])
def test_registry_gate_rejects_non_executing_verifier_wrappers(tmp_path: Path, wrapper: str) -> None:
    checker = runpy.run_path(str(ROOT / "scripts" / "checks" / "check_rule_verification.py"))
    checker["check_enforcement_wiring"].__globals__["ROOT"] = tmp_path
    command = "scripts/checks/example.py --check"
    (tmp_path / "pre.ps1").write_text(f"{wrapper} {command}\n", encoding="utf-8")
    (tmp_path / "ci.yml").write_text(f"steps:\n  - run: {wrapper} {command}\n", encoding="utf-8")
    data = {
        "pre_ci_entrypoint": "pre.ps1",
        "ci_workflow": "ci.yml",
        "rules": [{"id": "example", "verifier": ["scripts/checks/example.py", "--check"]}],
...
def test_canonical_whitespace_verifier_covers_worktree_index_and_committed_range() -> None:
    verify = (ROOT / "scripts" / "verify.ps1").read_text(encoding="utf-8")
    ci = (ROOT / ".github" / "workflows" / "ci.yml").read_text(encoding="utf-8")
    checker = (ROOT / "scripts" / "checks" / "check_git_whitespace.py").read_text(encoding="utf-8")
    assert "scripts/checks/check_git_whitespace.py" in verify
    assert "scripts/checks/check_git_whitespace.py" in ci
    assert '_require_clean_diff("--cached")' in checker
    assert "origin/main" in checker
