...
from commodity.eia import EiaBulkClient, capture_eia_api_dataset, capture_eia_bulk_dataset
...
def test_eia_bulk_capture_is_verifiable(tmp_path: Path) -> None:
    ...
    class Session:
        def get(self, url, timeout):
            return Response()
    manifest = capture_eia_bulk_dataset(EiaBulkClient(session=Session()), "NG", tmp_path, "snap", "2026-08-13T08:00:00Z")
...
def test_eia_api_capture_preserves_query_without_key(tmp_path: Path) -> None:
    ...
    manifest = capture_eia_api_dataset(Client(), "electricity/rto/region-data", {"facets[respondent][]": "US48"}, tmp_path, "snap", "2026-08-13T08:00:00Z")
