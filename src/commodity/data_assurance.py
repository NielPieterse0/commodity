...
def build_semantic_evidence(
    verifier: Callable[..., dict[str, bool]],
    *args: Any,
) -> dict[str, Any]:
    checks = verifier(*args)
    if not isinstance(checks, dict) or not checks:
        raise DataAssuranceError("semantic verifier must return explicit checks")
    source = inspect.getsourcefile(verifier)
    if not source:
        raise DataAssuranceError("semantic verifier source identity is unavailable")
    source_path = Path(source).resolve()
    if not source_path.is_file():
        raise DataAssuranceError("semantic verifier source file is unavailable")
    return {
        "method": "explicit_dataset_semantics_v1",
        "verifier_ref": f"{verifier.__module__}.{verifier.__qualname__}",
        "verifier_source_sha256": hashlib.sha256(source_path.read_bytes()).hexdigest(),
        "checks": checks,
    }
...
def verify_semantic_assurance(
    ...
    *,
    ...
    verifier_ref = semantic_evidence.get("verifier_ref")
    verifier_sha256 = semantic_evidence.get("verifier_source_sha256")
    if not isinstance(verifier_ref, str) or not verifier_ref:
        raise DataAssuranceError("semantic verification requires a bound verifier reference")
    if not isinstance(verifier_sha256, str) or len(verifier_sha256) != 64:
        raise DataAssuranceError("semantic verification requires verifier source identity")
    payload = {
        "method": method,
...
def assert_research_ready(assurance: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if assurance.get("reconstruction_independence") != "same_implementation_repeatability_only":
        raise DataAssuranceError("research dataset reconstruction independence is not explicitly characterized")
    ...
    verifier_sha256 = semantic_evidence.get("verifier_source_sha256")
    if not isinstance(verifier_ref, str) or not verifier_ref:
        raise DataAssuranceError("research dataset semantic evidence lacks verifier reference")
    if not isinstance(verifier_sha256, str) or len(verifier_sha256) != 64:
        raise DataAssuranceError("research dataset semantic evidence lacks verifier source identity")
