...
`V1` means the minimum serious dataset for the named layer. **U.S. / Henry Hub V1** is the completed evidence baseline; its final empirical disposition is recorded under `docs/development/v1-research-completion/`. Global/Interconnect and Norway/Europe remain later layers and are not activated by the V1 closeout.
...
| V1 | Issued weather forecasts | issue/available/valid time, lead, 2m temperature, HDD/CDD, wind, humidity, precipitation/snow; forecast revisions; demand-region anchors | Forecast run/hour -> daily features | Open-Meteo Single Runs for frozen V1; NOAA GFS for post-V2 revision research | The bounded V1 capture path remains unchanged and owns one 00 UTC ECMWF IFS run per day, four fixed U.S. gas-demand anchors, exact raw-response hashes, and deterministic 24-192h future-window aggregates. For a future post-V2 forecast-revision challenger, issue #114 establishes NOAA GFS 0.5-degree archived issued runs as the preferred baseline, with GEFS as a continuity-qualified secondary candidate and ECMWF MARS deferred pending access/cost approval. That follow-on is not activated. Current operational status and evidence are owned by `config/data_sources.json` and `docs/development/weather-revision-readiness/`. Never use reanalysis as a forecast substitute. |
| V1 | Underground storage | Lower-48 working gas, weekly change, release/revision event identity and point-in-time state | Week/event | EIA WNGSR `ngshistory.xls` + `revisions.xls`; API v2/bulk screening | The strict V1 path reconstructs the first-public value and later published revisions as a chronological state-event stream, preserving both raw workbook hashes. The release registry covers the required 2024-2026 window, including November/December 2024 holiday releases; the November 18, 2024 sample-reselection revision is a separate 15:00 ET event and never backfills earlier cutoffs. Current API/bulk history remains revised-current-state screening evidence. The V1 research-evaluation acquisition/readiness state is owned by `config/data_sources.json` and bound in `docs/development/v1-research-completion/phase-bc-empirical-closeout.json`. |
...
| Priority | Data family | Ideal datapoints | Grain | Preferred source | Access / point-in-time note |
|---|---|---|---|---|---|
| V1 | NCS gas production | field, NPDID information carrier, net saleable gas, gross gas, NCS total; revision/update timestamp | Month/field | Norwegian Offshore Directorate FactPages | Issue #115 verified deep public current-state history and dated preliminary aggregate releases, but not field-level historical vintages. Strict PIT activation is deferred; see `docs/development/norway-source-pit-readiness/`. |
| V1 | Norwegian gas transport | nominations/flow where public, receiving terminal, destination corridor, capacity and utilisation | Hour/day/point | Gassco + ENTSOG | Issue #115 verified Gassco's five-minute current/next-day nominations and only up-to-two-year final daily nomination history. ENTSOG is a machine-readable corroboration/control candidate, not a second independent signal; strict PIT activation remains deferred. |
| V1 | Norwegian maintenance/outages | planned/unplanned event, asset/field, unavailable capacity, start/end, update/cancellation history | Event/asset | Gassco FLOW/UMM | Issue #115 identifies timestamped Gassco event revisions as the strongest incremental Norway candidate, but historical event-vintage depth and approved automated acquisition remain unresolved. Capture-forward/archive verification is required before activation. |
| V1 | Gas exports | gaseous natural-gas export volume (Sm3), NOK value, partner country; derived unit value and destination mix | Month/country | SSB tables `08864` and `08799` via PxWebApi v2 | Public, no registration, CC BY 4.0, but current Statbank history is revision-bearing rather than a vintage archive. Issue #115 defers PIT use unless release vintages are reconstructed. |
...
| WX-ARCHIVE | [Open-Meteo](https://open-meteo.com/en/docs) + [NOAA GFS](https://www.ncei.noaa.gov/products/weather-climate-models/global-forecast) + [NOAA GEFS](https://www.ncei.noaa.gov/products/weather-climate-models/global-ensemble-forecast) | Public; ECMWF historical archive remains a separate access/cost decision | Actually-issued forecast vintages; Open-Meteo remains the frozen V1 path, while NOAA GFS is the preferred not-yet-activated post-V2 revision candidate |
