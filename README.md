...
- Market: CME Henry Hub natural-gas forecasting; `NG=F` is bootstrap research data only. Research backtesting is available on bootstrap/noncanonical inputs; promotion to canonical expiry-aware futures evidence remains pending an approved source. Saxo SIM is a read-only verification candidate and its chart data is not treated as official settlement.
